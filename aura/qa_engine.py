...
import torch
from .vision_text_bridge import load_model as load_vision_bridge
...
class QAEngine:
    ...
    def _load_vision_bridge(self):
        ...
        try:
            model_path = "models/vision_text_bridge.pt"
            if Path(model_path).exists():
                self.vision_bridge = load_vision_bridge(model_path)
    ...
    def _convert_embeddings_to_descriptions(self):
        ...
        try:
            for idx, (embedding, metadata) in enumerate(zip(self.embeddings, self.graph_metadata)):
                description, _ = self.vision_bridge.describe_embedding(embedding, metadata)
                self.graph_descriptions.append(description)
    ...
    def answer(self, question: str) -> str:
        """Answer user questions using Mistral with embedding context"""
        try:
